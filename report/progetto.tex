...
\end{align*}
In Figura 5 possiamo notare che vengono rispettate le specifiche 3 e 4 riguardo alla sovraelongazione percentuale e al tempo di assestamento.
...
infatti ha bande a bassa frequenza, mentre $n(t)$ lavora con bande ad alta frequenza. Il regolatore è stato costruito in modo da avere $S(j\omega)$ $\ll$ $1$ a basse frequenze e $F(j\omega) \ll 1$ ad alte frequenze, provando in questo modo ad abbattere entrambi i disturbi. I risultati ottenuti sono quindi in accordo con le specifiche del problema.
\section{Test sul sistema non lineare}

In questa sezione, testiamo l'efficacia del controllore progettato sul modello non lineare su Simulink.

In Figura 7, mostriamo lo schema a blocchi del sistema in anello chiuso. Per ottenere il sistema non linearizzato sono stati inseriti i seguenti componenti:
\begin{itemize}
...
Abbiamo esplorato il range di condizioni iniziali tali per cui l’uscita del sistema in anello chiuso converge a $h(x_e, u_e)$ e le abbiamo rappresentate tramite due heatmap in Figura 9: una che presenta anche il tempo di assestamento e una per la sovraelongazione. 
...
In particolare, abbiamo stabilità per sufficientemente grandi di $x_{2,e}$. All'aumentare di $x_{2,e}$, osserviamo un leggero aumento del tempo di assestamento e una diminuzione della sovraelongazione percentuale.\\

\begin{figure}[h]
    \centering
	\includegraphics [width=0.9\textwidth]{../figs/Esplorazione_range_equilibri.pdf} 
	\caption{Heatmap delle condizioni iniziali}
\end{figure}
...
Per valori inferirori, invece, il sistema torna a convergere, ma con tempi di assestamento molto dilatati e verso un valore pari circa a 16.

% inserire snapshot

\section{Punto opzionale: animazione}

Abbiamo realizzato in MATLAB un'animazione del sistema, graficando l'evoluzione lungo un periodo di %numero di secondi
secondi di $T_{out}$ rispetto al riferimento a gradino con ampiezza circa 80 e l'evoluzione dell'errore di regolazione. In tale animazione mostriamo anche la potenza $P_E$ in ingresso al sistema.
In Figura 11 abbiamo riportato uno schreenshot dell'animazione. 

\begin{figure}[h]
    \centering
	\includegraphics [width=0.9\textwidth]{../figs/schreen_animazione.jpeg} 
	\caption{Schreenshot dell'animazione}
\end{figure}
...
Iniziando con il lavoro svolto nel primo punto, attraverso un processo di linearizzazione nell’intorno del punto di equilibrio siamo riusciti ad analizzare un sistema non lineare. Trasformando quindi le equazioni che caratterizzano il sistema non lineare ci `e stato possibile studiarne il comportamento.
...
Infine i punti opzionali ci hanno permesso di esplorare in maniera più completa le performance del sistema non linearizzato modificando stato inziale e valore del gradino d’ingresso.
